...
public class AppStreamsManager {
    ...
    private static Stream<AppStreamActionDetails> getAppStreamsEnableDetails(Set<String> appStreamsToEnable) {
        return appStreamsToEnable.stream().map(
            appStream -> {
                String[] appStreamData = appStream.split(":");
                return AppStreamActionDetails.enableAction(
                        appStreamData[0],
                        appStreamData[1]
                );
            }
     ...
     */
    public static AppStreamModule findModule(Long channelId, String name, String stream) {
