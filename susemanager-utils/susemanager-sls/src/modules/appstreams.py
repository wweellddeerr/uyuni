...
def _parse_nsvca(module_info_output):
    nsvca_pattern = re.compile(r'^(?:Name\s+:\s+(\S+)\s*|Stream\s+:\s+(\S+)|Version\s+:\s+(\S+)|Context\s+:\s+(\S+)|Architecture\s+:\s+(\S+)).*$')
...
def enable(appstreams):
    ...
    '''
    cmd = ["dnf", "module", "enable", "-y"] + appstreams
    try:
...
def disable(appstreams):
    ...
    appstreams
        List of appstreams to disable
    
    Returns:
        Tuple: (result, comment, changes)
    '''
    cmd = ["dnf", "module", "disable", "-y"] + appstreams
    try:
        result = subprocess.run(cmd, check=False, capture_output=True)
        if result.returncode == 0:
