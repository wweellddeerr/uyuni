...
Feature: Install a package and a patch on the SUSE client via Salt through the UI
  ...
  Scenario: Pre-requisite: refresh package list and check old packages installed on SLE minion
    ...
    And I wait until package "andromeda-dummy-2.0-1.1" is removed from "sle_minion" via spacecmd
  ...
  Scenario: Install a patch on the SLE minion
    ...
    When I wait for "virgo-dummy-2.0-1.1" to be installed on "sle_minion"
